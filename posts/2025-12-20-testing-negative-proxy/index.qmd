...
$$
\exp\{\beta_{2A}\} = \frac{\exp\{\beta^*_{2A}\}}{\exp\{\beta^*_{1A}\}},
$$
...
The de-biased estimate is the difference between the two vaccination coefficients^[Note: difference-in-differences on the log scale is the same as a ratio of hazard ratios on the multiplicative scale]. In this run we get roughly `-0.49`, closer to the truth than the naive `-0.24`.
...
This is functionally equivalent to the classic test-negative design which only collects data from the tested.^[Indeed, one way to conceptualize the test-negative design is as a case-control study where cases and controls are concurrently sampled outcome-negative control outcome pairs. For more on this see: https://doi.org/10.1097/EDE.0000000000001926]
